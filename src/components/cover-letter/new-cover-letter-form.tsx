...
import { generateCoverLetter } from "@/app/[locale]/(app)/cover-letter/actions";
...
export function NewCoverLetterForm({ closeMode = "back" }: NewCoverLetterFormProps) {
    ...
    startTransition(async () => {
      try {
        ...
        if (closeMode === "replace") {
          router.replace(`/cover-letter/${result.token}`);
        } else {
  ...
  return (
    <div>
      ...
      <div className="mt-6 space-y-5">
        ...
        <div className="space-y-1.5">
          <Label>
            {t("jobPostingLabel")} <RequiredMark />
          </Label>
          <RichTextEditor
